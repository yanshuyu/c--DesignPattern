...
Rect::Rect(float w, float h, RendererAPI* renderer):Rect(w, h) {
    rendererImp = renderer;
}

Rect::Rect(const Point& o, float w, float h, RendererAPI* renderer):Rect(o, w, h) {
    rendererImp = renderer;
}
...
void Rect::draw() {
    if (rendererImp) {
        rendererImp->drawRect(origin, width, height);
    }
}
